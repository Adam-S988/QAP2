...
function parseDateString(input) {
  try {
    ...
    let [month, day, year] = value.split(/[\s-]+/);
    let months = {
      January: "01",
      February: "02",
      March: "03",
    ...
    let monthsNum = months[month] || month;
    let daysNum = days[day] || day;
    let date = new Date(`${monthsNum} ${daysNum} ${year}`);
    if (year.length !== 4) {
      throw new Error("Year must be displayed in 4 digits");
    }
    let X = `${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}`;
    return `${year}-${monthsNum}-${daysNum}`;
  } catch (error) {
...
console.log(mimeFromFilename("/User/Documents/song.mp3"));
...
function buildUrl(query, order, count, license) {
  try {
    let newQuery = query.replace(/\s+/g, "%20");
    if (order != "ascending" && order != "descending") {
      throw new Error("Order must be either ascending or descending");
    }
    if (count < 1 || count > 50) {
      throw new Error("Count must be between 1-50");
    }
    if (
      license != "none" &&
      license != "any" &&
      ...
      throw new Error("License must be a valid type.");
    }
    return `https://api.inaturalist.org/v2/observations?query='${newQuery}'&count=${count}&order=${order}&license=${license}`.toLowerCase();
  } catch (error) {
    console.error("An error occurred:", error.message);
    return null;
  }
